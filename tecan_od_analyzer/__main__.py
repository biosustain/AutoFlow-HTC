...
from tecan_od_analyzer.tecan_od_analyzer import argument_parser, gr_plots, \
    ...
    vol_correlation, compensation_lm, gr_estimation, estimation_writter, \
    stats_summary, input_output, stats_plot, step_gr_calculator
...
def main():
    ...
    try:
        ...
        sys.exit("Error: The output file from the autoflow_parser was not \
                 found")
    ...
    if not multiple_species_flag and not flag_bioshaker and \
            not flag_bioshakercolor:
        df_gr_final = reshape_dataframe(df_gr,
                                        flag_species=multiple_species_flag,
    ...
    for col in range(len(colnames)):
        my_series = pd.Series(data=(df_gr_final[colnames[col]]).tolist(),
                              index=df_gr_final[
                                  "time_"+colnames[col]].tolist())
        ...
        for cnt, rate in enumerate(rates):
            ...
            temp_step_df = pd.DataFrame({'Sample_name': [sample_name],
                                         'T2': [times[cnt]],
    ...
    writer = pd.ExcelWriter('Temporary_GR_check/stepwise_growth_rates.xlsx',
                            engine='xlsxwriter')
    ...
    if flag_all or flag_fig:
        ...
        if flag_ind:
            ...
            df_gr_est = \
                df_gr_final.loc[:, ~df_gr_final.columns.str.startswith('time')]
            colnames = (df_gr_est.columns.values)
                ...
                if not multiple_species_flag:
                    ...
                    for df_gr_est in df_gr_final_list:
                        ...
                        for col in range(len(colnames)):
                            ...
                            bioshaker_label = re.search(r"([B][S]\d)",
                                                        colnames[col]).group(1)
                            my_series = pd.Series(
                                data=(df_gr_final[colnames[col]]).tolist(),
                                index=df_gr_final[
                                    "time_" + colnames[col]].tolist())
                        ...
                        species_ = re.search(r"^\S{3}.\d?_?\S{2,3}_(\S+)",
                                             last_name).group(1)
