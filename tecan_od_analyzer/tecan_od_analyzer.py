...
def argument_parser(argv_list=None):
    ...
    parser.add_argument("-e", "--estimations", help="Get only the estimations \
                        for every sample", action="store_true")
    parser.add_argument("-f", "--figures", help="Get only the growth curve \
                        figures", action="store_true")
    parser.add_argument("-s", "--summary", help="Get only the summary of \
                        growth rate estimations", action="store_true")
    parser.add_argument("-it", "--interpolation", help="Get interpolation of \
                        growth rate measurements with given od",
                        action="store_true")
    parser.add_argument("-r", "--resultsdir", help="Name results directory",
                        action='store')
    parser.add_argument("-p", "--path", help="Path to data", action='store')

    # Visualization arguments
    parser.add_argument("-b", "--bioshaker", help="Get one growth rate figure \
                        for every individual bioshaker", action="store_true")
    parser.add_argument("-i", "--individual", help="Get one growth rate figure \
                        for every individual sample", action="store_true")
    parser.add_argument("-bc", "--bioshakercolor", help="Get one growth rate \
    ...
    parser.add_argument("-v", "--novolumeloss", help="Volume loss compesation \
                        is not computed", action="store_false")
    parser.parse_args()
...
def parse_data():
    ...
    try:
        ...
        return sys.exit("The data could not be parsed due to some error, check \
...
def read_xlsx(filename="results.xlsx"):
    ...
    cols = ["Sample ID", "Measurement", "Measurement type", "Sampling time",
            "Sampling date"]    # relevant variables
    df = df[cols]
...
def sample_outcome(sample_file, df):
    ...
    if IDs_parsed[0] not in IDs_calc and IDs_parsed[10] not in IDs_calc:
        ...
        print(f"""Warning: The format of the Sample IDs is not matching across \
                the input files:
            \n
        ...
        if re.search(r"BS\d[.]\d_[A-Z]\d", IDs_calc[0]) is not None:
            ...
            sys.exit("The ID format of the calc.tsv file could not be \
...
def compensation_lm(cor_df, df_gr, df_vl600):  # done
    ...
    for shaker in range(len(unique_bioshaker)):
        ...
        popt, pcov = curve_fit(linear, sub_cor_df["time_hours"],
                               sub_cor_df["Correlation"], p0=[1, 1])
        lm_eq.append(popt)

        ax = fig.add_subplot(math.ceil(len(unique_bioshaker)),
                             math.ceil(len(unique_bioshaker)/2), shaker+1)
        ax.plot(sub_cor_df["time_hours"], sub_cor_df["Correlation"], "o",
                label="Empirical data", markerfacecolor='skyblue',
                markeredgecolor="dodgerblue", alpha=0.5)
        ax.plot(sub_cor_df["time_hours"], linear(sub_cor_df["time_hours"],
                *popt), "b-", label="Linear model", color="darkred")
    ...
    for pos in range(len(lm_eq)):
        ...
        df_gr_background = df_vl600[df_vl600["bioshaker"] ==
                                    unique_bioshaker[pos]]
        sample_IDs = list(df_gr_comp['Sample_ID'].unique())
        for sample in sample_IDs:
            df_gr_comp_sample_limited = df_gr_comp[df_gr_comp['Sample_ID'] ==
                                                   sample]
            times = list(df_gr_comp['time_hours'][df_gr_comp['Sample_ID'] ==
                                                  sample].unique())
            for time in times:
                df_gr_comp_sample_time_limited = \
                    df_gr_comp_sample_limited[
                ...
                df_gr_comp["Measurement"].loc[df_gr_comp_index] = df_gr_comp[
                    "Measurement"].loc[df_gr_comp_index] - background_mean
        # 1 is added instead of the calculated intercept to only account
        # for the trend of volume loss and not an adjusted intercept
        df_gr_comp["Correlation"] = lm_eq[pos][0]*df_gr_comp["time_hours"] + 1
        df_gr_comp["Measurement"] = df_gr_comp["Measurement"] / \
...
def reshape_dataframe(df_gr, flag_species=False, flag_bioshaker=False):
    ...
    for i in range(len(unique_IDs)):
        ...
        n_list = df_gr.loc[df_gr["Sample_ID"] ==
                           unique_IDs[i], "Measurement"].tolist()
        column1 = pd.DataFrame({unique_IDs[i]: n_list})
        t_list = df_gr.loc[df_gr["Sample_ID"] ==
                           unique_IDs[i], "time_hours"].tolist()
        column2 = pd.DataFrame({"time_"+unique_IDs[i]: t_list})
        df_gr_final = pd.concat([df_gr_final, column1],
                                ignore_index=False, axis=1)
        df_gr_final = pd.concat([df_gr_final, column2],
                                ignore_index=False, axis=1)
    ...
    if not flag_species and not flag_bioshaker:
        ...
        df_gr_temp["Sample_ID"] = df_gr_temp["Sample_ID"] + "_" + \
            df_gr_temp["Species"]
        ...
        for pos in range(len(unique_bioshaker)):
            ...
            df_temp = df_gr[df_gr_temp.Sample_ID.str.contains(
                unique_bioshaker[pos])]
            ...
            for df in list_df_species:
                ...
                for i in range(len(unique_IDs)):
                    ...
                    m_list = df.loc[df["Sample_ID"] == unique_IDs[i],
                                    "Measurement"].tolist()
                    column1 = pd.DataFrame({unique_IDs[i]: m_list})
                    t_list = df.loc[df["Sample_ID"] == unique_IDs[i],
                                    "time_hours"].tolist()
                    column2 = pd.DataFrame({"time_"+unique_IDs[i]: t_list})
                    df_fin = pd.concat([df_fin, column1],
                                       ignore_index=False, axis=1)
                    df_fin = pd.concat([df_fin, column2],
                                       ignore_index=False, axis=1)
        ...
        if flag_bioshaker:
            ...
            for pos1 in range(len(unique_species)):
                ...
                for pos2 in range(len(unique_bioshaker)):
                    ...
                    df_temp = df_gr[df_gr.Species.str.contains(
                        unique_species[pos1])]
                    df_temp = df_temp[df_temp.Sample_ID.str.contains(
                        unique_bioshaker[pos2])]
...
def gr_estimation(df_gr_final):
    ...
    df_annotations["Parameter"] = ["Start", "End", "Slope", "Intercept", "n0",
                                   "SNR", "rank"]
    ...
    for col in range(len(colnames)):
        ...
        my_series = pd.Series(data=(
            df_gr_final[colnames[col]]).tolist(),
        ...
        df_temp = pd.DataFrame({colnames[col] + '_data': est_series.index,
                                colnames[col]+"_time": est_series.values})
        df_data_series = pd.concat([df_data_series, df_temp],
                                   ignore_index=False, axis=1)
        ...
        if colnames[col] in errors or len(gr_estimation[2]) == 0:
            ...
            df_annotations = pd.concat([df_annotations, df_temp_annotations],
                                       ignore_index=False, axis=1)
...
def gr_plots(df, sample, interpolationplot, color_=None, ind=False,
    ...
    if ind:
        ...
        if interpolationplot:
            try:
                ...
                a_BSpline = interpolate.make_interp_spline(df["time"],
                                                           df[sample])
        ...
        fig = plt.scatter(df["time"], df[sample], 5, facecolor=(.18, .31, .31),
                          label=sample)
        plt.plot(df["time"], df[sample], linestyle='-', color=(.18, .31, .31),
                 alpha=.25)
        plt.ylabel('Absorbance (OD)', fontname="Arial", fontsize=12)
        plt.xlabel('Time (h)', fontname="Arial", fontsize=12)
        plt.title("Growth curve of "+str(sample), fontname="Arial",
                  fontsize=12)
    ...
    # Create plots by combined by species
        ...
        if legend_ == "bioshaker":
            ...
            legend_label = re.search(r"^\S{3}.\d?_?\S{2,3}_(\S+)",
                                     sample).group(1)
        ...
        if title_ == "bioshaker":
            ...
            title_label = re.search(r"^\S{3}.\d?_?\S{2,3}_(\S+)",
                                    sample).group(1)

        elif title_ == "species_bioshaker":

            title_label = sample[:3]+"_"+re.search(
                r"^\S{3}.\d?_?\S{2,3}_(\S+)", sample).group(1)
        ...
        if separate_species:
            fig = plt.scatter(df["time"], df[sample], 5,
                              facecolor=(.18, .31, .31), color=color_,
                              label=legend_label, linestyle='-')
            plt.plot(df["time"], df[sample], linestyle='-', color=color_,
                     alpha=.1)
            plt.ylabel('Absorbance (OD)', fontname="Arial", fontsize=12)
            plt.xlabel('Time (h)', fontname="Arial", fontsize=12)
            plt.title("Growth curves of all species", fontname="Arial",
                      fontsize=12)
            plt.tight_layout()
            ...
            plt.ylabel('Absorbance (OD)', fontname="Arial", fontsize=12)
            plt.xlabel('Time (h)', fontname="Arial", fontsize=12)
            plt.title("Growth curve of "+str(title_label), fontname="Arial",
                      fontsize=12)
            plt.tight_layout()
...
def stats_summary(df_annotations):
    ...
    species_list = [re.sub(r'\S+[.]?\S+[_](\S+)', r'\1', i)
...
def interpolation(od_measurements, df_annotations, mean_df_bs):
    ...
    for sample_pos in range(len(estimated_samples_list)):
        ...
        if method == "well":
            ...
            parameters = df_annotations[
                estimated_samples_list[sample_pos]].tolist()
            estimation = exponential(time, parameters[3], parameters[2],
                                     parameters[4])
            ...
            df_temp = mean_df_bs.loc[(mean_df_bs['bioshaker'] ==
                                     (estimated_samples_list[sample_pos])[:3])]

            # subset to select appropriate species parameteres
            parameters = \
                df_temp.loc[df_temp['species'] ==
                            re.search(r"([A-Z0-9]+$)",
                                      estimated_samples_list[sample_pos]
                                      ).group(1)]

            # estimation
            estimation = exponential(time, parameters["intercep"].values[0],
    ...
    # Export to excel
    od_measurements.to_excel(r'interpolation_results.xlsx',
